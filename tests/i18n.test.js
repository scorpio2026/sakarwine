...
test('admin dashboard interior uses glass neon chrome and keeps desk wiring', () => {
  ...
  assert.match(accountsSlice, /accounts\.filter\(\(a\) => a\.isHost\)/);
  assert.match(accountsSlice, /accounts\.filter\(\(a\) => !a\.isHost\)/);
  assert.match(accountsSlice, /accountIdGroup\('hosts', t\('tabHosts'\), hosts\)/);
  assert.match(accountsSlice, /accountIdGroup\('users', t\('tabUsers'\), users\)/);
  assert.equal(accountsSlice.includes('<table>'), false);
  assert.equal(accountsSlice.includes('moderationButtons(a)'), false);
  assert.match(adminJs, /function accountIdChip/);
  assert.match(adminJs, /function accountIdGroup/);
  assert.match(adminJs, /account-id-list/);
  assert.match(adminJs, /account-id-chip/);
  assert.match(adminJs, /data-open-id/);
  assert.match(adminJs, /data-account-group/);
  assert.match(adminJs, /async function paintDossier/);
  assert.match(adminJs, /moderationButtons\(a\)/);
  assert.match(adminCss, /\.account-id-section\s*\{/);
  assert.match(adminCss, /\.account-id-heading\s*\{/);
  assert.match(adminCss, /\.account-id-list\s*\{/);
  assert.match(adminCss, /\.account-id-chip\s*\{[^}]*backdrop-filter:\s*blur/);
  assert.ok(I18n.catalogs.en.tabUsers);
  assert.ok(I18n.catalogs.en.accountListEmpty);
