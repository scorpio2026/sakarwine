...
- **Chat auto-translate:** when two people have different UI languages, each reads text in **their own language** (my / en / th / zh / ko / ja). The original and the sender’s language are stored; translations are cached server-side. Tap **Show original** on a translated bubble. System notes and voice are not translated; image captions (text only) are. Set `TRANSLATE_API_KEY` (Google Cloud Translation, or LibreTranslate via `TRANSLATE_URL`) on Render. If the key is missing, sakarwine tries a free MyMemory fallback, then shows the original.
